...
export function ItemMenu({
  ...
  /** Local verbs appended last (the builders' `extra` slot) — the album's
   *  Analyze audio lives here. */
  extraVerbs?: MediaMenuItem[];
}): React.JSX.Element {
  const { node } = menu;
  ...
  for (const p of presets?.presets ?? []) {
    if (p.id != null)
      occupied.set(p.id, {
        ...
        art: p.art_urls?.[0] ?? p.art_url ?? null,
      });
  }
          ...
          return (
            ...
            >
              {confirming ? (
                ...
                <ArtImage src={occ.art} lazy fallback={<span>{slot}</span>} />
              ) : (
                slot
              )}
            </button>
