...
const fmtDb = (g: number): string => `${g > 0 ? "+" : ""}${Number.isInteger(g) ? g : g.toFixed(1)}`;
...
export function ToneEq({ label = true }: { label?: boolean } = {}): React.JSX.Element | null {
  ...
  const tiltLive = useLiveWrite(
    (intensity) => void tt.command({ type: "setTiltIntensity", intensity }),
  );
        ...
        {caps.userEq && eq && (
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <HeaderChip
                onClick={() =>
                  void tt.command({ type: "setEqBands", gains: BAND_LABELS.map(() => 0) })
                }
            ...
            <div
              className={cx("flex items-end gap-1 transition-opacity", !eq.enabled && "opacity-60")}
            >
        ...
        {caps.tilt && tilt && (
          <div className="flex items-center gap-3 border-t border-edge pt-3.5" data-tilt>
            ...
            <div className={cx("flex-1 flex items-center gap-2.5", !tilt.enabled && "opacity-60")}>
              ...
              <span
                className="font-mono text-[11px] text-dim w-8 text-right shrink-0"
                data-tilt-value
              >
                {(() => {
