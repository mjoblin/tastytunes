 ...
 * dims instead).
 */
export async function openInfoForNowPlaying(playState: ZonePlayState | null, nowPlaying: ZoneNowPlaying | null): Promise<void> {
  ...
  const title = isRadio ? (md?.title ?? display?.line2 ?? md?.station ?? display?.line1 ?? null) : (md?.title ?? display?.line1 ?? null)
  ...
  const stub: MediaNode = {
    ...
    upnpClass: isRadio ? 'object.item.audioItem.audioBroadcast' : 'object.item.audioItem.musicTrack',
    isContainer: false,
  ...
  const local = !isRadio && !/airplay|cast|bluetooth|spotify|tidal|qobuz/i.test(`${klass ?? ''} ${stream.playbackSource ?? ''} ${stream.source ?? ''}`)
