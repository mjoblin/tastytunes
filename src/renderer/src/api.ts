 ...
 */
export type QueueOutcome = "ok" | "declined" | "failed";
export async function queueWrite(write: () => Promise<unknown>): Promise<QueueOutcome> {
  try {
    await write();
    return "ok";
  } catch (e) {
    return e instanceof QueueDeclined ? "declined" : "failed";
  }
}
