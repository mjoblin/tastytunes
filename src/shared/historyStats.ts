// The listening record's figures (0.8.0's History Stats), SHARED since 2026-09-14
// (the MCP round three): the Stats view, the Timeline's month dividers and the
// bridge's history_summary and history_top all read statsFor — one home for
// every number, so an agent's answer is the screen's. Pure over the events.
import { type ListeningEvent, type ListeningPlayEvent, isHiRes, isListen, playKey } from "./model";
