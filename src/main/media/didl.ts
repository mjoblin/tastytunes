...
const attr = (el: unknown, name: string): string =>
  el != null && typeof el === 'object' && !Array.isArray(el) ? String((el as Record<string, unknown>)[`@_${name}`] ?? '') : ''
...
const commaCount = (v: string): number => v.split(',').map((s) => s.trim()).filter(Boolean).length
...
function formatOf(res: Record<string, unknown> | undefined): MediaFormat | null {
  ...
  const kbps =
    fileKbps ??
    (bitrateAttr
      ? (() => { const asBytes = Math.round((bitrateAttr * 8) / 1000); return asBytes > 24_000 ? Math.round(bitrateAttr / 1000) : asBytes })()
      : undefined)
...
function nodeOf(raw: Record<string, unknown>, isContainer: boolean): MediaNode | null {
  ...
  if (albumArtist == null && roleless && performerText.length === 1 && creator != null && creator.toLowerCase() !== performerText[0].toLowerCase()) {
    albumArtist = performerText[0]
    performerText = [creator]
  }

  let artist: string | null = (performerText.length > 0 ? performerText.join('; ') : null) ?? creator ?? albumArtist ?? text(artistEls[0]) ?? null
  ...
  if (!isContainer && artists.length === 1 && albumArtist && artist && artist.toLowerCase().startsWith(`${albumArtist.toLowerCase()}, `)) {
  ...
  for (const name of own) {
    if (title.length > name.length + 3 && title.toLowerCase().startsWith(`${name.toLowerCase()} - `)) {
      title = title.slice(name.length + 3).trim()
      break
    }
  }
  if (isContainer && year && title.endsWith(` (${year})`)) title = title.slice(0, -year.length - 3).trim()

  const trackNumber = raw.originalTrackNumber != null ? Number(text(raw.originalTrackNumber)) : null
  return {
...
export function didlToNodes(didl: string): MediaNode[] {
  ...
  if (!root) return []
  const containers = asArray(root.container as Record<string, unknown> | Array<Record<string, unknown>>)
    .map((c) => nodeOf(c, true))
